...
In the literature, the most common way to construct an index using PCA is to use the 1st PC. However, in our case, the 1st PC only explained 14% of the variance. So, I would suggest using weighted average of the PCs, where the weights can be determined based on expert opinions.
...
1.  PC1, it is more urban, dense, well-developed.
2.  PC2, it has more economic and digital resources, higher education levels, and younger population.
3.  PC3, it has more access to health services, younger and more mobile population, and greater digital inclusion.
4.  PC4, it has more native population, more adults, digitally connected, mobile population (with cars).
5.  PC5, it has more stable, English proficient, and younger residents, more of which are families.
...
So, we can observe that Housing units, Seniors, Poverty status, and No internet are the contributing variables to the index of capacity module in terms of the first two PCs. Next, we visualize the raw distribution of the index, which turns out to be highly left-skewed. To make the index more interpretable, we consider a transformation guided by Box-Cox transformation. Finally, we standardize the index.
...
To make the results more interpretable, we can compute the contribution of the variables below. Note that Poverty status, Lack of Geographical mobility, Minority, Seniors, No Internet, No Computer, and Housing units are the variables with high contribution.
...
Then, we look at the distribution of the clusters and index for the Buncombe County.
