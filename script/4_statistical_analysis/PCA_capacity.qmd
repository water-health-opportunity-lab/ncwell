...
```{r}
# =============================================================
# Constrained PCA with partial sign constraints via directionality
# directionality: +1 (force >=0), -1 (force <=0), NA (no constraint)
# =============================================================

# ---- (internal) solve first constrained PC by projected power + multi-start
.constrained_pc1 <- function(S, Jpos = integer(), Jneg = integer(),
  ...
  for (rs in 1:nstart) {
    ...
    for (it in 1:maxit) {
      v <- S %*% u                            # power step (gradient)
      if (length(Jpos)) v[Jpos] <- pmax(v[Jpos], 0)  # enforce ≥ 0
      if (length(Jneg)) v[Jneg] <- pmin(v[Jneg], 0)  # enforce ≤ 0
      nv <- sqrt(sum(v^2))
      if (nv == 0) {                          # degenerate; reinit
        u <- rnorm(p); u <- u / sqrt(sum(u^2))
        next
      }
      u_new <- v / nv
      cur_val <- drop(t(u_new) %*% S %*% u_new)
      if (abs(cur_val - prev_val) < tol) {    # convergence
        u <- u_new
        break
      }
      u <- u_new
...
# ---- main: constrained PCA with optional directionality vector
constrained_pca <- function(X,
                            directionality = NULL,   # +1, -1, NA per variable (length p)
                            Jpos = integer(),        # optional explicit indices for >=0
                            Jneg = integer(),        # optional explicit indices for <=0
                            ncomp = 2,
                            center = TRUE, scale. = TRUE,
                            maxit = 2000, tol = 1e-8, nstart = 10,
                            drop_tol = 1e-10) {

  # 0) scale data like prcomp
  Xs <- scale(X, center = center, scale = scale.)
  S  <- crossprod(Xs) / nrow(Xs)                 # correlation if scale.=TRUE
  p  <- ncol(S)

  # 1) interpret directionality (if given); explicit Jpos/Jneg override NAs
  if (!is.null(directionality)) {
    if (length(directionality) != p)
      stop("Length of 'directionality' must equal ncol(X).")
    Jpos_dir <- which(directionality > 0)
    Jneg_dir <- which(directionality < 0)
  } else {
    Jpos_dir <- integer(0); Jneg_dir <- integer(0)
  }
  # merge with explicit Jpos/Jneg arguments (union)
  Jpos <- sort(unique(c(Jpos_dir, Jpos)))
  Jneg <- sort(unique(c(Jneg_dir, Jneg)))

  # 2) extract components with deflation
  U    <- matrix(NA_real_, p, ncomp)
  vals <- numeric(ncomp)
  Sdef <- S

  for (k in 1:ncomp) {
    pc1  <- .constrained_pc1(Sdef, Jpos = Jpos, Jneg = Jneg,
                             maxit = maxit, tol = tol, nstart = nstart)
    U[, k]   <- pc1$u
    vals[k]  <- pc1$value
    # spectral (rank-1) deflation
    Sdef <- Sdef - vals[k] * (pc1$u %*% t(pc1$u))
  }

  # 3) order by variance explained (descending) and drop ~zero
  ord     <- order(vals, decreasing = TRUE)
  U       <- U[, ord, drop = FALSE]
  vals    <- vals[ord]
  keep    <- which(vals > drop_tol)
  if (length(keep) == 0L) {
    warning("All components have ~zero variance under constraints; returning empty model.")
    keep <- 1L
  }
  U    <- U[, keep, drop = FALSE]
  vals <- vals[keep]

  # 4) compute scores on the scaled data
  scores <- Xs %*% U

  # 5) return prcomp-like list (not classed yet)
  list(
    loadings = U,                      # p x q (q = kept components)
    sdev     = sqrt(pmax(vals, 0)),    # standard deviations per component
    x        = scores,                 # n x q scores
...
# ---- helper: cast to "prcomp" so you can use fviz_eig() etc.
as_prcomp_constrained <- function(fit) {
  out <- list(
    sdev     = fit$sdev,
    rotation = fit$loadings,
    center   = fit$center,
    scale    = fit$scale,
    x        = fit$x
  )
...
pca <- constrained_pca(
  ...
  ncomp = ncol(scaled_data),
  nstart = 30
)

# scree plot
library(factoextra)
fviz_eig(as_prcomp_constrained(pca), addlabels = TRUE,
         barfill = "white", barcolor = "magenta") +
...
rot <- as_prcomp_constrained(pca)$rotation
...
So, we can observe that plumbing, pregnancy, and housing units are the contributing variables to the index of capacity module in terms of the first two PCs.

Next, we visualize the raw distribution of the index, which turns out to be highly left-skewed. To make the index more interpretable, we consider a transformation guided by Box-Cox transformation. Finally, we standardize the index.

```{r}
# Eigenvalues (variance explained by each PC)
eigenvalues <- pca$sdev^2

# Proportion (fraction) of total variance explained
...
cum_pve_percent <- cumsum(pve_percent)

## raw distribution of the index
which(cum_pve_percent > 80)

## raw distribution of the index
# index <- apply(PCs[, 1:min(which(cum_pve_percent > 60))], 1, mean)
# index <- PCs[, c(4, 6, 8, 9, 10)] %*% matrix(c(0.04910, 0.00265, 0.32185, 0.03342, 0.10147))
index <- PCs[, c(2, 4, 8, 10)] %*% matrix(c(0.011978, 0.077763, 0.354246, 0.123984))
hist(index, main = 'Raw Capacity Index', col = 'white',
     border = 'magenta')

## transformation
lmod <- lm(index - min(index) + 1e-3 ~ 1)
bc <- boxcox(lmod)
...
st_write(data[, c(1, 2, 4, 29, 28)], 'index_capacity.gpkg',
         layer = "my_layer", driver = "GPKG", append = FALSE)
out <- data %>% select(-c(block_group_name))
out <- cbind(data, PCs[, 1:10])
colnames(out)[(ncol(out) - 9):(ncol(out))] <- paste0('cap_PC', 1:10)
st_write(out, 'index_capacity_detailed.gpkg',
         layer = "my_layer", driver = "GPKG", append = FALSE)
```
...
# View contribution
cont <- round(apply(contrib[, 1:3], 1, sum), 2)
cont <- cont / sum(cont) * 100
cont[order(cont, decreasing = TRUE)[1:10]]
```

Next, we visualize the sum of squared loadings of the ten most important variables (measured in terms of Contribution) in a barplot below.

```{r}
library("stringr")
idx <- order(cont, decreasing = TRUE)[1:10]   # get top 3 directly
...
p <- ggplot(df, aes(x = reorder(Variable, cont.subset), y = cont.subset, fill = cont.subset)) +
  ...
  theme(
    ...
    axis.title.x = element_text(color = "black", face = "bold", size = 22),
    axis.title.y = element_text(color = "black", face = "bold", size = 22),
    plot.title   = element_text(color = "black", face = "bold",
                                hjust = 0.5, size = 22),
    legend.title = element_text(color = "black", face = "bold", size = 22),
    legend.text  = element_text(size = 12, color = "black"),
    legend.position = "none"
  )

ggsave(filename = "capacity_import.png",
       plot = p,
