...
In the literature, the most common way to construct an index using PCA is to use the 1st PC. However, in our case, the 1st PC only explained 18.2% of the variance. So, I would suggest using weighted average of the PCs, where the weights can be determined based on expert opinions.
...
## health insurance (+), government jobs (+), new homes (+), no internet (-)
w3 <- round(rot[, 3], 2)
w3[which(abs(w3) > 0.3)]
...
leaflet(data_watauga, options = leafletOptions(minZoom = 7)) %>%
  setView(-81.7349, 36.1942, zoom = 10) %>%
...
leaflet(data_mecklenburg, options = leafletOptions(minZoom = 7)) %>%
  setView(-80.8544, 35.2633, zoom = 9) %>%
