...
Next, we interpret the PCs as follows.
...
# matrix(c(0.00103, 0.01605))
index <- PCs[, c(5, 6)] %*% matrix(c(0.099985, 0.082707))
...
if (lambda > 0) {
  index <- (index - min(index)) ^ (as.numeric(lambda))
} else {
  index <- log(index - min(index) + 1e-3)
}
...
out <- cbind(data, PCs[, 1:6])
colnames(out)[(ncol(out) - 6):(ncol(out) - 1)] <- paste0('haz_PC', 1:6)
st_write(out, 'index_hazard_detailed.gpkg',
         layer = "my_layer", driver = "GPKG", append = FALSE)
```
...
# View contribution
cont <- round(apply(contrib[, 1:5], 1, sum), 2)
...
var_names[c(7, 6, 5)] <- c('Satelite-based Inundation Map', 'TRI Water Impact', 'TRI Total Impact')
