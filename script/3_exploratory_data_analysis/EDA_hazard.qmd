...
2.  We found many outliers for (1) Satellite-based inundation map, (2) CAFOs, (3) TRI water impact, (4) TRI total impact, (5) Onsite Wastewater Treatment Systems (OWTS), and (6) Agricultural Runoff. It appears that applying square root transformation helps resolve this issue except for OWTS and CAFOs. Instead, a log transformation works for OWTS and log10 transformation works for CAFOs.
...
Next, we check if square root transformation helps. The idea is to cover a desired proportion of data (say, 98%) of normally behaved data under some transformation. As we can see below, square root transformation helps.
...
oc.check(data$inundation, method = 'sqrt')
...
```{r}
ggplot(data = data) + geom_sf(aes(color = sqrt(inundation))) + 
  theme_bw() + labs(title = "Satellite-based inundation map") + 
  theme(panel.grid = element_blank()) +
  scale_color_gradient(low = "deepskyblue", high = "magenta", 
                       name = '',
...
Similarly, we check whether we can apply some transformation to avoid this issue. For OWTS, we notice that log transformation works while square root transformation does not work. For agricultural runoff, square root transformation works while log transformation does not work
...
ggplot(data = data) + geom_sf(aes(color = log(owts + 1e-4))) + 
  ...
  scale_color_gradient(low = "deepskyblue", high = "magenta", 
                       name = '',
                       na.value = 'grey')
ggplot(data = data) + geom_sf(aes(color = sqrt(ag_runoff))) + 
  theme_bw() + labs(title = "Agricultural runoff (Square root)") + 
  theme(panel.grid = element_blank()) +
  scale_color_gradient(low = "deepskyblue", high = "magenta", 
                       name = '',
                       na.value = 'grey')
...
```{r}
vifs <- c()
vars <- as.data.frame(data[, -c(1:2, 11)])
vars <- vars[, -9]
for (j in 1:8) {
  y.j <- unlist(vars[, j])
  x.j <- vars[, -j]
