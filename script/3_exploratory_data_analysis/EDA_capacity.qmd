...
data$pct_minority[is.na(data$pct_minority) == TRUE]
...
data$pct_limited_english <- ifelse(data$pct_limited_english > 0, 1, 0)
...
ggplot(data = data) +
  geom_sf(aes(color = as.factor(pct_limited_english), 
              fill = as.factor(pct_limited_english)), size = 0.5) +
...
data$pct_single_parent <- log(data$pct_single_parent + 1e-4)
data$pct_unemployed <- log(data$pct_unemployed + 1e-4)
data$pct_no_college <- coding(sqrt(data$pct_no_college), tail = 'right')
```
...
# replacement
data$pct_women <- log(data$pct_women + 1e-4)
data$pct_over_65 <- log(data$pct_over_65 + 1e-4)
data$pct_women_gave_birth <- log(data$pct_women_gave_birth + 1e-4)
data$pct_insured <- coding(log(data$pct_insured + 1e-4), 
                           tail = 'left')
```
...
data$pct_us_born <- coding(log(data$pct_us_born + 1e-4), tail = 'left')
```
...
data$pct_owner_occupied <- log(data$pct_owner_occupied + 1e-4)
data$pct_active_commuting <- log(data$pct_active_commuting + 1e-4)
data$pct_no_move <- coding(sqrt(data$pct_no_move), tail = 'left')
```
...
data$pct_gov_workers <- sqrt(data$pct_gov_workers)
```
...
data$pct_crowded_housing <- log(data$pct_crowded_housing + 1e-4)
data$pct_plumbing <- log(data$pct_plumbing + 1e-4)
data$pct_new_home <- sqrt(data$pct_new_home)
```
...
data$pct_no_vehicle <- log(data$pct_no_vehicle + 1e-4)
data$pct_no_internet <- sqrt(data$pct_no_internet)
data$pct_no_computer <- sqrt(data$pct_no_computer)
data$pop_density <- log(data$pop_density + 1e-4)
data$housing_units_density <- log(data$housing_units_density + 1e-4)
data$households_density <- log(data$households_density + 1e-4)
```
...
x.trans <- cbind(x1.trans, x2.trans, 
                 ...
                 x7.trans, x8.trans)
# print 
data_transform <- data[, - c(26, 28)]

st_write(data_transform, "nc_grid_capacity_transformed_no_impute.gpkg", 
...
# }
# 
# st_write(data_transform, "nc_grid_capacity_transformed.gpkg", 
#          layer = "my_layer", driver = "GPKG", append = FALSE)
```
