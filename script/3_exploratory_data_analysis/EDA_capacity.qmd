...
1.  We found outliers in some variables. Applying square root transformation or log transformation helps. However, the outliers may still be alarming after transformation. In this case, top coding (or winsorization) helps. It is defined as setting the values above 98% percentile as 98 percentile and those below 2% percentile as 2%. The histograms are provided in this case.
2.  It may be a better idea to transform English proficiency into a binary variable as it is zero-inflated and transformation does not help.
3.  There exists multicollinearity among the set of transformed variables. If we remove Households, Population size, and Internet from the set of variables, the multicollinearity problem will not be of concern.
...
In terms of English proficiency, we check its distribution on the right tail below. It may be a good idea to transform it into an ordered binary variable. Or alternatively, we may check if some transformation and top coding works. As we can observe, this approach does not appear effective.
...
# Education
x1.2.trans <- top.code(sqrt(data$pct_limited_english))
oc.check(x1.2.trans, method = 'power', degree = 1) * 100
hist(x1.2.trans, col = 'white', border = 'magenta',
     main = 'English Proficiency', xlab = '')
x1.trans <- race
```
...
ggplot(data = data) + geom_sf(aes(color = pct_limited_english)) + 
  theme_bw() + labs(title = "English proficiency") + 
  theme(panel.grid = element_blank()) +
  scale_color_gradient(low = "deepskyblue", high = "magenta", 
                       name = '',
                       na.value = 'grey')
...
# Education
x2.3.trans <- top.code(sqrt(data$pct_no_college))
...
We try using top coding to see if it helps. According to the histogram below, we can see that the transformed variable appears more well-behaved.

```{r}
x3.5.trans <- top.code(log(data$pct_insured + 1e-4))
...
Similarly, we check whether applying some transformation helps. As we may observe, there are still some outliers. We check if top coding helps.
...
As we can see, after top coding, the distribution of the transformed Nativity looks more well-behaved.

```{r}
x4.trans <- top.code(log(data$pct_us_born + 1e-4))
...
Next, we check if top coding works for Geographical mobility. As we can see, the transformed variable looks more well-behaved.

```{r}
x5.3.trans <- top.code(sqrt(data$pct_no_move + 1e-4))
...
As we can observe, there exist serious multicollinearity among those transformed variables. Next, we try to get a set of variables with VIFs smaller than 10. If we remove Households, Population size, and Internet from the set of variables, the multicollinearity problem will not be of concern.
