...
microbio_pooled_affected <- rbind(microbio_pooled_hurricane, microbio_pooled_microbiology_affected) %>%
  filter(!duplicated(StarLiMS_ID)) %>%
  filter(System_Name %in% counties)
```

Compare risk indices of western counties to the rest of North Carolina
...
After filtering the dates to the effective time period, there were 3433 samples in `microbio_pooled_filtered` (all locations) and 874 samples for `microbio_pooled_affected_filtered` (western counties).

We observe:

-   3350 uncontaminated and 83 contaminated E. coli samples in `microbio_pooled_filtered`

-   839 uncontaminated and 33 contaminated E. coli samples in `microbio_pooled_affected_filtered`

-   2470 uncontaminated and 963 contaminated Total Coliform samples in `microbio_pooled_filtered`

-   603 uncontaminated and 269 contaminated Total Coliform samples in `microbio_pooled_affected_filtered`.
