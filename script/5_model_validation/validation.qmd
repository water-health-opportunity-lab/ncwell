...
```{r}
# ----------------------------------------
# R Script to validate overall risk index using NCDHHS well testing results for E. coli and Total Coliform
...
n_pre_filter_hurricane <- nrow(microbio_with_risk_hurricane)
...
n_post_filter_hurricane <- nrow(microbio_with_risk_hurricane)
...
n_pre_filter_microbio <- nrow(microbio_with_risk_microbiology)
...
n_post_filter_microbio <- nrow(microbio_with_risk_microbiology)
```

`{r} n_pre_filter_hurricane - n_post_filter_hurricane` samples were filtered out from the Hurricane Helene dataset. `{r} n_pre_filter_microbio - n_post_filter_microbio` samples were filtered out from the general microbiology dataset.
...
# Check for unique values
table(microbio_pooled$Ecoli_Result_Binary)
table(microbio_pooled$Coliform_Result_Binary)
```

#### Filter for only western counties
...
n_microbio_pooled_affected <- nrow(microbio_pooled_affected)
```

After filtering for only western counties, there were `{r} n_microbio_pooled_affected` samples remaining.

#### Wilcoxon rank-sum test for risk index of western vs. non-western counties
...
wilcox.test(microbio_pooled$risk ~ microbio_pooled$Is_Western, alternative = "less")

ggplot(microbio_pooled) +
  aes(x = Is_Western, y = risk, color = Is_Western) +
  ggdist::stat_halfeye(aes(fill = Is_Western),
...
When all considering regions with low private well usage, there is not a significant difference between the risk indices of samples collected in western counties vs. samples collected in the rest of North Carolina (p = 0.5232), with samples from the western counties having a mean risk of 0.265 and non-western counties having a mean risk of 0.264 The risk index geopackage has since been updated, so the tables and plots above exclude regions with low private well usage.
...
Analyzing the plots from section 1.4, we see that the level of microbial contamination falls to its lowest point in early March. Here, we filter samples collected after 3/1/25, the end of the effective time period.
...
summary(microbio_pooled_filtered$risk[microbio_pooled_filtered$Is_Western == TRUE])
summary(microbio_pooled_filtered$risk[microbio_pooled_filtered$Is_Western == FALSE])

wilcox.test(microbio_pooled_filtered$risk ~ microbio_pooled_filtered$Is_Western, alternative = "less")

ggplot(microbio_pooled_filtered) +
  aes(x = Is_Western, y = risk, color = Is_Western) +
  ggdist::stat_halfeye(aes(fill = Is_Western),
                       adjust = 0.5, width = 0.6, .width = 0, 
                       justification = -0.3) + 
...
n_microbio_pooled_filtered <- nrow(microbio_pooled_filtered)
n_microbio_pooled_affected_filtered <- nrow(microbio_pooled_affected_filtered)

table_filtered_ecoli <- table(microbio_pooled_filtered$Ecoli_Result_Binary)
table_affected_filtered_ecoli <- table(microbio_pooled_affected_filtered$Ecoli_Result_Binary)

table_filtered_coliform <- table(microbio_pooled_filtered$Coliform_Result_Binary)
table_affected_filtered_coliform <- table(microbio_pooled_affected_filtered$Coliform_Result_Binary)
```

After filtering the dates, there was not a significant difference in the risk index of samples taking from western and non-western counties. After also filtering for western counties, there were `{r} n_microbio_pooled_affected_filtered` samples in `microbio_pooled_affected_filtered` (western counties).

We observe:

-   `{r} table_filtered_ecoli["Absent"]` uncontaminated and `{r} table_filtered_ecoli["Present"]` contaminated E. coli samples in `microbio_pooled_filtered`

-   `{r} table_affected_filtered_ecoli["Absent"]` uncontaminated and `{r} table_affected_filtered_ecoli["Present"]` contaminated E. coli samples in `microbio_pooled_affected_filtered`

-   `{r} table_filtered_coliform["Absent"]` uncontaminated and `{r} table_filtered_coliform["Present"]` contaminated Total Coliform samples in `microbio_pooled_filtered`

-   `{r} table_affected_filtered_coliform["Absent"]` uncontaminated and `{r} table_affected_filtered_coliform["Present"]` contaminated Total Coliform samples in `microbio_pooled_affected_filtered`.

Downstream analysis will be performed using `microbio_pooled_affected_filtered`.

### 2.1.1 Risk index distribution for E. coli in western counties

Based on the Shapiro-Wilk tests, we reject the null hypothesis that the sample of risk indices for uncontaminated samples comes from a normal distribution (p = 5.43e-08), but we fail to reject the null hypothesis for contaminated samples (p = 0.01064). However, looking at the raincloud plot for contaminated samples, we see it is non-normal.
...
### 2.1.2 Wilcoxon rank-sum test for E. coli in western counties

There is not a significant difference between the mean risk index for locations with uncontaminated vs. contaminated E. coli tests (p = 0.4003).

```{r}
wilcox.test(microbio_pooled_affected_filtered$risk ~ microbio_pooled_affected_filtered$Ecoli_Result_Binary, alternative = "less")
```

We can also test whether the distributions of the risk score differ between the contaminated group and uncontaminated group, using Kolmogorov-Smirnov test.

```{r}
# Kolmogorov-Smirnov test
y <- microbio_pooled_affected_filtered$risk
x <- microbio_pooled_affected_filtered$Ecoli_Result_Binary
ks.test(y ~ x, alternative = "less")
```

We fail to reject the null hypothesis that their distributions are similar.

### 2.2.1 Risk index distribution for Total Coliform in western counties

Based on the raincloud plots and Shapiro-Wilk tests, we reject the null hypothesis that the sample of risk indices for both uncontaminated (p = 2.099e-06) and contaminated samples (p = 0.0002728) comes from a normal distribution.
...
### 2.2.2 Wilcoxon rank-sum test for Total Coliform in western counties

There is not a significant difference between the mean risk index for locations with uncontaminated vs. contaminated Total Coliform tests (p = 0.714).

```{r}
wilcox.test(microbio_pooled_affected_filtered$risk ~ microbio_pooled_affected_filtered$Coliform_Result_Binary, alternative = "less")
```
...
# Kolmogorov-Smirnov test
y <- microbio_pooled_affected_filtered$risk
x <- microbio_pooled_affected_filtered$Coliform_Result_Binary
ks.test(y ~ x, alternative = "less")
```
...
For E. coli contamination, the overall risk was not a significant predictor (p = 0.667). In the model with the individual risk modules, the physical vulnerability module was marginally significant (p = 0.0994) and the social capacity module was significant (p = 0.0330). The model using overall risk as a sole predictor (AIC = 311.46) performed marginally better than the module using individual risk modules (AIC = 308.43).

For Total Coliform contamination, the overall risk was not a significant predictor (p = 0.685). In the model with the individual risk modules, the physical vulnerability module was highly significant (p = 0.00863), the capacity module was significant (p = 0.03994), and the hazard module was marginally significant (p = 0.09755). Like E. coli contamination, the model using overall risk as a sole predictor (AIC = 773.42) performed marginally better than the module using individual risk modules (AIC = 766.68).
...
coefplot(multi_log_coliform)

# capacity_log_coliform <- glm(Coliform_Result_Binary_Model ~ index_capacity, data = microbio_pooled_affected_filtered, family = "binomial")
# summary(capacity_log_coliform)
```

Since the logistic regression produced negative coefficients for physical vulnerability, contrary to common intuition, we can impose sign constraints to ensure that the estimated coefficients align with theoretical expectations. This can be achieved by fitting a constrained logistic regression model.

```{r}
library("glmnet")
contam <- ifelse(microbio_pooled_affected_filtered$Coliform_Result_Binary == 'Present', 1, 0)
X_cols <- c("index_vulnerability", "index_hazard", "index_capacity")
y <- contam
dat <- microbio_pooled_affected_filtered


loglik_binom <- function(y, p, eps = 1e-12) {
  # Stable binomial log-likelihood
  p <- pmin(pmax(p, eps), 1 - eps)
  sum(y * log(p) + (1 - y) * log(1 - p))
}

fit_constrained_logit <- function(X, y, lower, upper, standardize = TRUE) {
  # Unpenalized (lambda = 0) constrained logistic regression via glmnet
  glmnet(
    x = X, y = y,
    family = "binomial",
    lambda = 0,
...
# Build reduced model by dropping j-th column
drop_term <- function(X, lower, upper, j) {
  list(
    Xr = X[, -j, drop = FALSE],
    lr = lower[-j],
    ur = upper[-j]
...
# Design matrix (no intercept) and 0/1 outcome
X <- model.matrix(~ index_vulnerability + index_hazard + index_capacity, data = dat)[, -1, drop = FALSE]

# Constraints:
#   beta_vulnerability >= 0
#   beta_hazard        >= 0
#   beta_capacity      <= 0
lower_full <- c(0,   0,   -Inf)
upper_full <- c(Inf, Inf,   0)

# Fit full model and compute observed full log-likelihood
fit_full <- fit_constrained_logit(X, y, lower_full, upper_full)
p_full   <- as.numeric(predict(fit_full, newx = X, s = 0, 
                               type = "response"))
ll_full  <- loglik_binom(y, p_full)

# permutation
perm_LRT_term <- function(X, y, lower_full, upper_full, ll_full, j, B = 2000, seed = 123) {
  set.seed(seed)

  # Reduced (drop jth) under observed labels
  parts <- drop_term(X, lower_full, upper_full, j)
  fit_r <- fit_constrained_logit(parts$Xr, y, parts$lr, parts$ur)
  p_r   <- as.numeric(predict(fit_r, newx = parts$Xr, s = 0, type = "response"))
  ll_r  <- loglik_binom(y, p_r)
  ...
  for (b in seq_len(B)) {
    ...
    # Full under permuted labels
    fit_f_p <- fit_constrained_logit(X, y_perm, lower_full, upper_full)
    p_f_p   <- as.numeric(predict(fit_f_p, newx = X, s = 0, type = "response"))
    ll_f_p  <- loglik_binom(y_perm, p_f_p)

    # Reduced under permuted labels
    fit_r_p <- fit_constrained_logit(parts$Xr, y_perm, parts$lr, parts$ur)
    p_r_p   <- as.numeric(predict(fit_r_p, newx = parts$Xr, s = 0, type = "response"))
    ll_r_p  <- loglik_binom(y_perm, p_r_p)

    perm_stats[b] <- 2 * (ll_f_p - ll_r_p)
  }

  pval <- mean(perm_stats >= obs)
  list(obs_LRT = unname(obs), p_value = pval, null = perm_stats)
}

# 4) RUN TERM-WISE TESTS ------------------------------------------------------
term_names <- colnames(X)
results <- lapply(seq_along(term_names), function(j) {
  out <- perm_LRT_term(X, y, lower_full, upper_full, ll_full, j, B = 2000, seed = 123 + j)
  data.frame(
    term   = term_names[j],
    obs_LRT = out$obs_LRT,
    p_value = out$p_value,
...
print(coef(fit_full, s = 0))

# p-value
print(results)
```

Based on the permutation test, the social capacity index is statistically significant and exhibits a negative association with contamination risk, as expected. The model assigned a zero coefficient to the physical vulnerability index to satisfy the imposed constraint, while the hazard index was not found to be significant.

## 4. K-fold cross validation for logistic regression
...
# Reformat outcome as factor with levels "Absent"/"Present"
microbio_pooled_affected_filtered$Coliform_Result_Binary_Model <-
  factor(microbio_pooled_affected_filtered$Coliform_Result_Binary_Model,
         levels = c(0, 1), labels = c("Absent", "Present"))

# Train with caret
cv_model <- train(
  Coliform_Result_Binary_Model ~ index_vulnerability + index_hazard + index_capacity,
  data = microbio_pooled_affected_filtered,
  method = "glm",
  family = "binomial",
...
## 5. Youden's J statistic for risk index split

Youden's J statistic was used to compute the cutoff threshold for contaminated vs. uncontaminated samples. The cutoff was 0.2103 for E. coli and 0.1648 for Coliform.
...
## 6. Save microbio_pooled_affected_filtered as GPKG for Figure 4

```{r}
st_write(microbio_pooled_affected_filtered, 'well_tests_filtered.gpkg', layer = "my_layer", driver = "GPKG", append = FALSE)
```
