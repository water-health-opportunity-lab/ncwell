...
microbio_continuous <- microbio_with_risk %>%
  filter(!(Ecoli_Result %in% c("Absent", "Present"))) %>%
  mutate(Ecoli_Result_Binary = ifelse(Ecoli_Result %in% c("0", "<1"), "Absent", "Present")) %>%
  mutate(Coliform_Result_Binary = ifelse(Total_Coliform_Result%in% c("0", "<1"), "Absent", "Present"))
```
...
There is a significant difference between the risk indices of samples collected in western counties vs. samples collected in the rest of North Carolina (p = 0.0012), with samples from the western counties having a mean risk of 0.181 and non-western counties having a mean risk of 0.175.
...
After filtering the dates, there was a significant difference the risk index of samples taking from western and non-western counties. After also filtering by western vs. non-western, there were `{r} n_microbio_pooled_filtered` samples in `microbio_pooled_filtered` (all locations) and `{r} n_microbio_pooled_affected_filtered` samples for `microbio_pooled_affected_filtered` (western counties).
...
Based on the Shapiro-Wilk tests, we reject the null hypothesis that the sample of risk indices for uncontaminated samples comes from a normal distribution (p = 5.625e-09), but we fail to reject the null hypothesis for contaminated samples (p = 0.7522). However, looking at the raincloud plot for contaminated samples, we see it is non-normal.
...
There is a marginally significant difference between the mean risk index for locations with uncontaminated vs. contaminated E. coli tests (p = 0.07162).
...
Based on the raincloud plots and Shapiro-Wilk tests, we reject the null hypothesis that the sample of risk indices for both uncontaminated (p = 3.541e-07) and contaminated samples (p = 0.002128) comes from a normal distribution.
...
There is not marginally significant difference between the mean risk index for locations with uncontaminated vs. contaminated Total Coliform tests (p = 0.2258).
...
For E. coli contamination, the overall risk was not a significant predictor (p = 0.244). In the model with the individual risk modules, none of the modules were significant predictors. The model using overall risk as a sole predictor (AIC = 252.48) performed marginally better than the module using individual risk modules (AIC = 254.67).

For Total Coliform contamination, the overall risk was not a significant predictor (p = 0.419). In the model with the individual risk modules, the capacity module was a highly significant predictor (p = 0.00316). Unlike for E. coli contamination, the model using individual risk modules (AIC = 978.6) performed better than the module using overall risk (AIC = 983.27).
